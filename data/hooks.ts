...
return <div>Mouse: {x}, {y}</div>`,
...

    {
        ...
        parameters: [],
        returns: [
            { name: 'currentTheme', type: 'Theme', description: 'Current theme object with colors, styles' },
            { name: 'setTheme', type: '(id: string) => void', description: 'Function to change the active theme' },
            { name: 'themes', type: 'Theme[]', description: 'Array of all available themes' },
        ],
        example: `const { currentTheme, setTheme, themes } = useTheme()
