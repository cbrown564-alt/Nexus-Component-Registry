...
const Toggle = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
    <div className="flex items-center justify-between p-3 bg-slate-900/50 border border-blue-900/30 rounded">
        <span className="text-xs font-mono text-blue-200">{label}</span>
        <button 
            onClick={onClick}
            className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${active ? 'bg-blue-600' : 'bg-slate-700'}`}
        >
...
const SystemControls = () => {
  ...
  return (
    <GridCard title="System Controls" className="space-y-4">
