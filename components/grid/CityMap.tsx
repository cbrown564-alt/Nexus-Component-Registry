...
const CityMap = () => {
  ...
  return (
    <GridCard className="h-full min-h-[500px] p-0" title="Geospatial Overview">
      <div className="relative h-full w-full bg-[#0b1121] overflow-hidden">
        ...
        {zones.map((zone) => (
            ...
            >
                {/* Ping Animation */}
                <div className={`absolute inset-0 animate-ping rounded-full h-4 w-4 opacity-75 ${
                    zone.status === 'critical' ? 'bg-red-500' : 
                    zone.status === 'warning' ? 'bg-amber-500' : 'bg-emerald-500'
                }`} />
                
                {/* Node Icon */}
                <div className={`relative flex h-4 w-4 items-center justify-center rounded-full border border-black shadow-lg ${
