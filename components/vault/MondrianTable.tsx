...
export interface Protocol {
    ...
    apy: string;
    tvl: string;
    risk: 'MIN' | 'LOW' | 'MED' | 'HIGH';
}

export interface MondrianTableProps {
    /** Array of protocol data to display */
    protocols?: Protocol[];
    /** Label for the detail button */
    buttonLabel?: string;
    /** Callback when the button is clicked */
    onViewAll?: () => void;
    /** Highlight percentage to show in corner block */
...
const getRiskColor = (risk: Protocol['risk']) => {
    switch (risk) {
        case 'MIN': return 'bg-blue-500/10 text-blue-500 border-blue-500';
        case 'LOW': return 'bg-green-500/10 text-green-500 border-green-500';
        case 'MED': return 'bg-yellow-500/10 text-yellow-500 border-yellow-500';
        case 'HIGH': return 'bg-red-500/10 text-red-500 border-red-500';
    }
};

const MondrianTable: React.FC<MondrianTableProps> = ({
        ...
        <div className="grid grid-cols-4 grid-rows-3 gap-[1px] bg-[#333] border border-[#333] font-mono text-xs h-[400px]">
            ...
            <div className="col-span-1 row-span-2 bg-[#0a0a0a] p-4 flex flex-col justify-between border-l border-[#333]">
                <div>
                    ...
                    <span className={`px-2 py-1 rounded-sm border ${getRiskColor(protocols[0]?.risk ?? 'LOW')}`}>
                        {protocols[0]?.risk ?? 'LOW'}
                    </span>
                </div>
            ...
            <div className="col-span-3 row-span-1 bg-black p-4 border-t border-[#333] flex justify-between items-center">
                ...
                <button
                    onClick={onViewAll}
                    className="text-[10px] uppercase border border-white/20 px-3 py-1 hover:bg-white hover:text-black transition-colors"
                >
                    {buttonLabel}
                </button>
            </div>
            ...
            <div className="col-span-1 row-span-1 bg-green-500 p-4 flex items-center justify-center">
                <span className="text-black font-black text-xl">{highlightValue}</span>
            </div>
        </div>
    );
};
