    ...
    <GameCard variant="accent" className="p-5">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-bold text-white">Active Quests</h3>
        </div>
      ...
      <div className="space-y-4">
        {quests.map((quest, i) => (
            <div key={i} className="rounded-xl bg-black/20 border border-amber-500/10 p-3">
                <div className="flex justify-between items-start mb-2">
                    <h4 className={`font-bold text-sm ${quest.completed ? 'text-zinc-500 line-through' : 'text-amber-100'}`}>
                        {quest.title}
                    </h4>
                    {quest.completed ? (
                ...
                <div className="flex items-center justify-between">
                     ...
                     {quest.completed && (
                         <button className="text-[10px] font-bold bg-emerald-500 hover:bg-emerald-400 text-white px-3 py-1 rounded shadow-lg shadow-emerald-500/20 transition-all active:scale-95">
                             Claim
                         </button>
                     )}
                </div>
