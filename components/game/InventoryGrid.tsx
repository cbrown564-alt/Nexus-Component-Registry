        ...
        <GameCard variant="secondary" className="p-5 h-full">
            ...
            <div className="grid grid-cols-4 gap-3">
                {items.map((item, i) => (
                    <div
                        key={i}
                        className={`aspect-square rounded-xl border-2 flex items-center justify-center relative group transition-all duration-200 cursor-pointer ${item
                    ...
                    >
                        {item && (
                            <>
                                <div className={`absolute inset-2 opacity-20 blur-md ${item.bg || 'bg-zinc-500'}`} />
                                <item.icon
                                    className={`relative z-10 h-6 w-6 drop-shadow-md group-hover:scale-110 transition-transform ${item.color}`}
                                    style={{ color: item.rarity === 'common' ? theme.colors.foreground : undefined }}
                                />
