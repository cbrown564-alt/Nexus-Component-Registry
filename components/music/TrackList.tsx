    ...
    <MusicCard className="flex flex-col h-full p-0 bg-transparent border-none shadow-none backdrop-blur-none">
       ...
       <div className="flex-1 overflow-y-auto">
           {tracks.map((track, i) => (
               <div 
                key={i} 
                className={`group flex items-center px-6 py-3 hover:bg-white/5 transition-colors cursor-pointer rounded-lg mx-2 ${
                    track.active ? 'bg-white/10' : ''
                }`}
               >
                   ...
                   <div className="flex-1 flex items-center gap-4">
                       <img 
                        src={`https://source.unsplash.com/random/40x40?abstract&sig=${i}`} 
                        alt="Art" 
