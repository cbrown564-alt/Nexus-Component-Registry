...
export interface GrowthChartProps {
    ...
    /** SVG path for the growth curve (for advanced customization) */
    customPath?: string;
}

const defaultDataPoints: ChartDataPoint[] = [
    { x: 400, y: 220, label: 'Q1' },
    { x: 700, y: 150, label: 'Q2' },
    { x: 1100, y: 100, label: 'Q3' },
    { x: 1600, y: 50, label: 'Q4' },
];

const GrowthChart: React.FC<GrowthChartProps> = ({
    title = 'Carbon Sequestration',
    subtitle = 'Yearly Impact (Tons)',
    dataPoints = defaultDataPoints,
    customPath,
}) => {
    // Generate path from data points if no custom path provided
    const path = customPath || "M 0 250 Q 200 200 400 220 T 700 150 T 1100 100 T 1600 50";

    return (
        <OrganicContainer className="h-[400px] w-full relative overflow-hidden" variant="glass">
            {/* Header */}
            <div className="absolute top-8 left-8 z-10">
                <h3 className="font-serif text-3xl text-[#1a2e1a]">{title}</h3>
                <p className="text-[#8f9e8a] text-sm uppercase tracking-widest font-bold mt-1">{subtitle}</p>
            </div>
            ...
            <div className="absolute inset-x-0 bottom-0 h-3/4 w-full">
                <svg viewBox="0 0 1600 300" className="w-full h-full overflow-visible" preserveAspectRatio="xMidYMid slice">
                    ...
                    {dataPoints.map((point, i) => (
                        ...
                        >
                            <circle cx={point.x} cy={point.y} r="6" fill="#fbbf24" stroke="#fdfcf8" strokeWidth="2" />
                            {/* Little Leaf Icon SVG */}
                            <foreignObject x={point.x - 12} y={point.y - 30} width="24" height="24">
